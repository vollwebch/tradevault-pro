        ...
        <div className="p-4 md:p-6 max-w-7xl mx-auto">
          ...
          {page==='dashboard'&&(
            <div className="space-y-6">
              ...
              {initCapital>0&&<Card className="bg-gradient-to-r from-[#e31937]/20 to-[#e31937]/5 border-[#e31937]/30"><CardContent className="p-4 flex flex-wrap gap-4"><div><p className="text-xs text-zinc-400">Balance Actual</p><p className="text-xl font-bold">{fmt$(totalBal)}</p></div><div><p className="text-xs text-zinc-400">Capital Inicial</p><p className="text-lg font-semibold">{fmt$(initCapital)}</p></div><div><p className="text-xs text-zinc-400">ROI</p><p className="text-lg font-semibold" style={{color:roi>=0?'#00c853':'#e31937'}}>{roi}%</p></div></CardContent></Card>}
              {stats.totalTrades>0&&(
                ...
                <Card className="bg-[#111] border-[#222]"><CardHeader><CardTitle className="text-sm">Trades Recientes</CardTitle></CardHeader><CardContent><div className="space-y-2">{trades.slice(0,5).map(t=>(<div key={t.id} className="flex items-center gap-3 p-2 rounded-lg bg-[#1a1a1a] text-sm cursor-pointer hover:bg-[#222]" onClick={()=>openTrade(t)}><span className={`font-bold ${calcPnL(t)>=0?'text-[#00c853]':'text-[#e31937]'}`}>{fmt$(calcPnL(t))}</span><span className="text-zinc-300">{t.symbol}</span><Badge variant={t.direction==='LONG'?'default':'secondary'} className={t.direction==='LONG'?'bg-[#00c853]/20 text-[#00c853]':'bg-[#e31937]/20 text-[#e31937]'}>{t.direction}</Badge><span className="text-zinc-500 ml-auto">{fmtDateShort(t.date)}</span></div>))}{trades.length===0&&<p className="text-zinc-500 text-sm text-center py-4">No hay trades todavia</p>}</div></CardContent></Card></>
