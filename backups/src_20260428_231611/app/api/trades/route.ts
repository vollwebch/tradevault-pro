...
function mapTrade(row: Record<string, unknown>) {
  return {
    ...
    brokerFees: row.broker_fees,
    profitSplit: row.profit_split,
    realPnl: row.real_pnl,
    setup: row.setup,
    notes: row.notes,
    emotion: row.emotion,
...
export async function POST(request: NextRequest) {
  try {
    ...
    db.prepare(`
      INSERT INTO trades (id, user_id, date, symbol, direction, entry_price, exit_price, stop_loss, take_profit, shares, commission, broker_fees, profit_split, real_pnl, setup, notes, emotion, screenshot, tags)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id,
      decoded.userId,
      ...
      body.brokerFees ?? null,
      body.profitSplit ?? 100,
      body.realPnl ?? 0,
      body.setup || null,
      body.notes || null,
      body.emotion ?? null,
